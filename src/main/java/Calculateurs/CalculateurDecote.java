...
public class CalculateurDecote {
    ...
    /**
     * Calcule la décote de l'impôt sur le revenu en fonction des revenus nets
    ...
    public double calculerDecote(double impotBrut, SituationFamiliale situationFamiliale) {
        ...
        if (situationFamiliale == SituationFamiliale.MARIE
            ...
            if (impotBrut < Parametres2024.SEUIL_DECOTE_COUPLE) {
                decote = Parametres2024.DECOTE_MAX_COUPLE -
                        (impotBrut * Parametres2024.TAUX_DECOTE);
            }
        } else {
            if (impotBrut < Parametres2024.SEUIL_DECOTE_CELIBATAIRE) {
                decote = Parametres2024.DECOTE_MAX_CELIBATAIRE -
                        (impotBrut * Parametres2024.TAUX_DECOTE);
            }
        }
